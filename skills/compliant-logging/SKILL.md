...
description: >-
  ...
  EVENT (SOC 2 CC7.2 / CC7.3) and that secrets and PII never leak into logs (SOC 2 CC6.x).
  Applies in any stack; do not invent a new logging library — use the project's logger.
license: Apache-2.0
---

# Compliant logging (SOC 2 CC7.2 / CC6.x) — authoring-time guidance

> Scope: this skill helps you *implement* the audit-logging and log-hygiene controls in code.
> It does **not** make a system "SOC 2 compliant" — certification is the auditor's call. It makes
> the code satisfy the technical control and produces machine-detectable evidence of that.
...
| `control` | the SOC 2 control id, e.g. `"CC7.2"` |
...
## Log hygiene — secrets & PII must never enter logs (CC6.x)
...
def delete_user(actor_id: str, user_id: str, correlation_id: str) -> None:
    try:
        ...
        logger.info(
            "audit",
            audit=True, control="CC7.2", action="user.delete",
...
async function grantRole(actorId: string, userId: string, role: string, correlationId: string) {
  const base = {
    audit: true, control: "CC7.2", action: "role.grant",
